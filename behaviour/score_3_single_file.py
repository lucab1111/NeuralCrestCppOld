...
def single_file_count(cell_position_df, cellradius, PMZheight, Clength):

    # Change df columns to numeric where appropriate

    # Look at correct chain
    double_file_count = 0
            ...
            if time_step > 0:
                ...
                for index, row in this_time_step_df.iterrows():
                    ...
                    if np.isnan(prev_x) and np.isnan(prev_y): # First cell
                        ...
                        inter_cell_Y_distance = math.sqrt( math.pow((prev_y - this_y), 2) )

                        if inter_cell_Y_distance < cell_diameter/2:
                            #print('#At time ' + str(time_step) + ' Distance between cells ' + str(inter_cell_Y_distance))
                            double_file_count += 1

                        total_time_step_count += 1

                    prev_x = this_x
                    prev_y = this_y

    if total_time_step_count > 0:
        return double_file_count/ total_time_step_count
            ...
            try:
                ...
                if not len(initial_conditions) == 28:
                    ...
                    if not os.path.isfile(filename+'/celldf_with_first_cell_every_nth.csv'):
                        ...
                        ratio_double_file_count = single_file_count(cellpositions_df, cellradius, PMZheight, Clength)
                        ...
                        data = {
                                ...
                                'ratio_double_file_count': ratio_double_file_count,
...
result_df.to_csv(CURRENT_DIR + '/behavioural_data/result_3_single_file_count' + str(ARRAY_INDEX) + '.csv')
