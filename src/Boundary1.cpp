...
void Boundary1(vector<cell>& Cells,parameters& params,float& gamma){
  ...
  float diamondOffset = (params.CMwidth-params.CEwidth)/2.0;
    ...
    if ((Cells[j].pos(0) + params.PMZheight/2.0) < Cells[j].cellradius){
      ...
      if ((Cells[j].pos(0) + params.PMZheight/2.0) > 0){
        ...
        if (fabs(Cells[j].pos(1)) > params.CEwidth/2.0 && fabs(Cells[j].pos(0)+params.PMZheight/2.0) < fabs(Cells[j].pos(1))-params.CEwidth/2.0){
          ...
          if (Cells[j].pos(0) > (diamondOffset - params.Clength - params.PMZheight/2.0) || Cells[j].pos(0) < -(diamondOffset + params.Clength + params.PMZheight/2.0)){
            // Straight section before and after diamond
            dx.zeros();
            dx(1) = -1.0;
            ...
            // Diamond section
            dx.zeros(); // Refresh boundary force direction
            q = 2.0*diamondOffset*(params.CEwidth/2.0 - Cells[j].cellradius*sqrt((params.CMwidth-params.CEwidth)*(params.CMwidth-params.CEwidth)/(4.0*diamondOffset*diamondOffset) +1.0))/(params.CMwidth-params.CEwidth) - params.PMZheight/2.0;
            p = 2.0*diamondOffset/(params.CMwidth-params.CEwidth);
            yleft = p*Cells[j].pos(1) + q;
            if ((Cells[j].pos(0)+params.Clength-diamondOffset) > yleft) {
              //Interacts with top left side of chain zone
              dx(0) = params.CMwidth-params.CEwidth;
              dx(1) = -2.0*diamondOffset;
              dx_mag = sqrt(dot(dx,dx));
              dy = Cells[j].pos(0) + params.Clength - diamondOffset - yleft;
              dif = -dy*(params.CMwidth-params.CEwidth)/sqrt(4.0*diamondOffset*diamondOffset + (params.CMwidth-params.CEwidth)*(params.CMwidth-params.CEwidth));
              F = (dx*2.0*De*a*(exp(-a*dif)-exp(-2.0*a*dif)))/dx_mag;
              Cells[j].v = Cells[j].v + F/gamma;
              Cells[j].v_bound = Cells[j].v_bound + F/gamma;
            }
            ...
            dx.zeros(); // Refresh boundary force direction
            q = 2.0*diamondOffset*(Cells[j].cellradius*sqrt((params.CMwidth-params.CEwidth)*(params.CMwidth-params.CEwidth)/(4.0*diamondOffset*diamondOffset) +1.0) - params.CMwidth/2.0)/(params.CMwidth-params.CEwidth) - params.PMZheight/2.0 - diamondOffset;
            p = -2.0*diamondOffset/(params.CMwidth-params.CEwidth);
            yleft = p*Cells[j].pos(1) + q;
