...
data = np.genfromtxt(argv[2]+"/cellpositions.txt")
conditions = np.genfromtxt(argv[2]+"/initialconditions.txt")
nloop = int(argv[1])
...
if visualisationMode == 0:
    ...
    circles = [matplotlib.patches.Circle((xi,yi), radius=zi, alpha=0.5,edgecolor="black") for xi,yi,zi in zip(data[nloop*nc:(nloop+1)*nc,2],data[nloop*nc:(nloop+1)*nc,1],data[nloop*nc:(nloop+1)*nc,7])]
    for c in circles:
        ax.add_patch(c)
...
fig.savefig(argv[2]+"/plot{:05d}.png".format(nloop),bbox_inches='tight',padding_inches=0,dpi=200,format='png')
ax.cla()
