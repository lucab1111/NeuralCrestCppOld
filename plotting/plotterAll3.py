...
for ii in range(Iterations):
    ...
    if chainShape == 3:
        ...
        # Cover part of the mz
        left_mz = matplotlib.patches.Rectangle((-PMZwidth/2, -PMZheight/2 - Clength - PMZheight*mz_to_pmz_height_ratio),
        width = PMZwidth/2 - 0.3,
        height= PMZheight*mz_to_pmz_height_ratio,
        alpha= alpha,
        edgecolor='k',
        fc = 'k')
        ax.add_patch(left_mz)

                # Cover part of the mz
        right_mz = matplotlib.patches.Rectangle(( PMZwidth/2 - (PMZwidth/2 - 0.3), -PMZheight/2 - Clength - PMZheight*mz_to_pmz_height_ratio),
        width = (PMZwidth - CEwidth)/3,
        height= PMZheight*mz_to_pmz_height_ratio,
        alpha= alpha,
        edgecolor='k',
        fc = 'k')
        ax.add_patch(right_mz)
        ...
        # End of the migration iee 120 microns from top of PMZ
    ...
    if visualisationMode == 0:
        ...
        for index, row in timeframe.iterrows():
            ...
            outercircle = matplotlib.patches.Circle((xi,yi), 
                                    radius=zi, 
                                    alpha=alpha, 
                                    fill=False,
                                    edgecolor="grey",
                                    linewidth=1.5,
                                    facecolor=color_list[int(ci)])
    ...
    fig.savefig(argv[1]+"/plot{:05d}.png".format(ii),bbox_inches='tight',dpi=300,format='png')
    ax.cla()
