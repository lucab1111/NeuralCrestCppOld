...
for ii in range(Iterations):
    ...
    if chainShape == 3:
        ...
        # pmz_entry_left = matplotlib.patches.Rectangle((-PMZwidth/2, -PMZheight/2 - Clength/10),
        # width = (PMZwidth - CEwidth)/2,
        # height= Clength/10,
        # alpha= alpha,
        # edgecolor='w',
        ...
        # Initiation of cell being out of the PMZ
        ylevel = (-PMZheight/2.0-Clength/10.0)
        ...
        l, = plt.plot(x,y, '--', c = 'deepskyblue')

        # End of the migration ie 120 microns from top of PMZ

        outer_threshold = -1.845
        ...
        l2, = plt.plot(x2,y2, '--', c = 'magenta')


        middle_zone_middle = (-PMZheight/2.0-Clength -PMZheight*mz_to_pmz_height_ratio/2) 


        x3 = [-3,3]
        y3 = [middle_zone_middle,middle_zone_middle]

        dashes = [5,2,10,5] # 5 points on, 2 off, 3 on, 1 off

        l3, = plt.plot(x3,y3, '--', c = 'springgreen')


        top_pmz = (+PMZheight/2.0) 
        ...
        l4, = plt.plot(x4,y4, '--', c = 'darksalmon')


        print((middle_zone_middle - PMZheight/2)*57)
        print((outer_threshold - PMZheight/2)*57)
    ...
    if visualisationMode == 0:
        ...
        for index, row in timeframe.iterrows():
            ...
            circle = matplotlib.patches.Circle((xi,yi), radius=zi/3, alpha=alpha,edgecolor="black",facecolor=color_list[int(ci)])
            ax.add_patch(circle)

            outercircle = matplotlib.patches.Circle((xi,yi), radius=zi, alpha=alpha, fill=False,edgecolor="grey",facecolor=color_list[int(ci)])
